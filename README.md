...
참고:
- 인메모리 모드에서는 두 계정 모두 기본값으로 보입니다.
- PostgreSQL 자동 시드 로직은 기본적으로 관리자 계정 중심으로 생성됩니다.
- 이 부분은 아직 `buildflow.com` 기준 더미 값이 남아 있으므로, 실제 운영 계정 체계와 맞추려면 별도 정리가 필요합니다.
